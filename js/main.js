...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function showFormMessage(message, type) {
    ...
    messageDiv.style.cssText = `
            ...
            ${
              type === "error"
                ? "background: #ffe6e6; color: #cc0000; border: 1px solid #ff4444;"
                : ""
            }
            ${
              type === "success"
                ? "background: #e6f7e6; color: #006600; border: 1px solid #4CAF50;"
                : ""
            }
        `;
  ...
  // ========== ANIMATE NUMBERS ON SCROLL (FIXED WITH SYMBOL SUPPORT) ==========

  // New function (keeps %, /7 etc.)
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (
          ...
          // Get number (first digits)
          const numberMatch = value.match(/\d+/);
          if (!numberMatch) return;

          const number = parseInt(numberMatch[0]);

          // Get suffix (like %, /7, + etc.)
          const suffix = value.replace(numberMatch[0], "");

          el.classList.add("animated");

          animateNumberWithSuffix(el, number, suffix);
        }
      });
    },
    { threshold: 0.5 }
  ...
  console.log(
    "%cSGD Shipping Website",
    "font-size: 24px; font-weight: bold; color: #D4AF37;"
  );
  console.log(
    "%cPremier Barge Rental Services in Chittagong, Bangladesh",
    "font-size: 14px; color: #001F3F;"
  );
  console.log(
